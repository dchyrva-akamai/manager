...
Qwen3 4B is a dense instruction-tuned model from Alibaba's Qwen team. Despite its compact size, it rivals the performance of Qwen2.5-72B-Instruct on many benchmarks. [[1]](#ref-1) A key innovation is its **hybrid thinking system**: a single model that seamlessly switches between a step-by-step reasoning mode (for math, coding, and complex logic) and a fast non-thinking mode (for general dialogue and simpler tasks). [[2]](#ref-2)
...
| Benchmark | Notes |
|---|---|
| General Reasoning | Rivals Qwen2.5-72B-Instruct despite 18x fewer parameters |
| Math | Strong results in thinking mode; competitive with much larger models |
| Coding | Leading performance among open models at this parameter count |
| Agent Tasks | Top open-source performance on complex agentic benchmarks |
...
<a id="ref-3"></a>**[3]** Qwen Team — *Qwen3-4B Model Card* (Hugging Face)  
