...
// Simulate token newly generated by a sharegroup consumer that has not yet been accepted by the sharegroup owner
export const newlyGeneratedSharegroupTokenFactory =
  Factory.Sync.makeFactory<SharegroupToken>({
    created: new Date().toISOString(),
    expiry: new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toISOString(),
    label: Factory.each((id) => `sharegroup-token-${id}`),
    sharegroup_label: null as unknown as string,
